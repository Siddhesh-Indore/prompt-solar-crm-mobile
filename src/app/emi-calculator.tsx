...
// of promising to "check and call back." Standard reducing-balance EMI
// formula, plus a year-by-year amortization breakdown (the "advanced" part
// — how much of each year's payments actually goes to principal vs interest).
...
function computeEmi(principal: number, annualRatePct: number, tenureYears: number) {
  ...
  for (let y = 0; y < Math.ceil(tenureYears); y++) {
...
export default function EmiCalculatorScreen() {
  ...
  const result = useMemo(() => {
    ...
    return computeEmi(amount, rate, tenureYears)
