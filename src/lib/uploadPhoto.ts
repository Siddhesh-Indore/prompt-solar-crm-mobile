...
// Shared compress+upload core behind uploadVisitPhoto.ts and
// uploadIntakePhoto.ts — same storage layout ({execId}/{leadId}/{filename}),
// just a different bucket per feature.
//
...
export async function uploadPhoto(bucket: string, execId: string, leadId: string, localUri: string, suffix = ''): Promise<string> {
  const base64 = await compressPhoto(localUri)

  const path = `${execId}/${leadId}/${Date.now()}${suffix}.jpg`
